...
my (%gene_words_to_annotate, %common_words_to_add, %taxonomy, %special_first_terms);

## REFACTOR: This is a temporary solution. Create a database table with these values ASAP
$special_first_terms{"anti"} = "anti";
...
sub getAnnotations
{
	...
	foreach(keys %{$self->{_searchWords}})
	{
		...
		GENE_CHECK:
		{
			## Disregard any words that are over 10 characters
			...
			{ 
				...
				{ 
					...
					{ 
						...
						## Check whether the term exists in the special first words hash...
						if (exists $special_first_terms{$_}) { $term_found = "true"; }
					} 
					else 
					{ 
						my $retType = ref($tag);
						$term_found = "true";
						if ($retType eq "word_search::DBResult" )
						{
						...
						## If the return type is drug add to the annotated drugslist
						elsif ($retType eq "word_search::Drug" ){ $self->{_geneAnnotation}->addAnnotatedDrug($tag);	}
						else { ; }
					}  
	...
	## Compile the return object that consists of genes and drugs
	my %returnObject;
	$returnObject{"genes"} = $retHash;
	$returnObject{"drugs"} = $self->{_geneAnnotation}->getAnnotatedDrugWords();
	return \%returnObject;
}	
